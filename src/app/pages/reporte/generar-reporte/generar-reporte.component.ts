...
export class GenerarReporteComponent implements OnInit {
  ...
  @ViewChild('inputProducto', { static: true }) inputProducto:InputFormComponent;
 ...
 /**
  ...
  protected setInitialValue() {
    ...
    this.filteredProductos
      ...
      .subscribe(() => {
        this.singleSelect2.compareWith = (a: any, b: any) => a && b && a.id === b.id;
      });
  }
  ...
  protected filterClientes() {
    ...
    const f = this.items.filter(function (d) {
      if (d.nombreEmpresa != '' && d.nombreEmpresa!=null && d.nombreEmpresa!=undefined) {
        return (
  ...
  protected filterProductos() {
    ...
    const f = this.productos.filter(function (d) {
      if (d.descripcion != '' && d.descripcion!=null && d.descripcion!=undefined) {
        return (
            d.descripcion.toLowerCase().indexOf(search) !== -1);
  ...
  updateSelectClientes(e){
    ...
    let clienteTemp = this.items.filter(res=>{return (res.nombreEmpresa==this.cliente.nombreEmpresa);});
  ...
  calculate(){
    ...
    this.productosSeleccionados.forEach((e,index) => {
      console.log(this.reporte.seguro,this.productosSeleccionados[index].total,this.totalBruto);
      let multiSeguro = (this.productosSeleccionados[index].total*parseInt(this.reporte.flete+''));
      console.log("MULTI:"+(multiSeguro/parseFloat(this.totalBruto+'')),parseFloat(this.totalBruto+''));
      this.productosSeleccionados[index].flete = this.productosSeleccionados[index].total*(parseFloat(this.reporte.flete+''))/parseFloat(this.totalBruto+'');
      this.productosSeleccionados[index].seguro = this.productosSeleccionados[index].total*(parseFloat(this.reporte.seguro+''))/parseFloat(this.totalBruto+'');
      this.totalFob =this.totalFob+this.productosSeleccionados[index].fob;
      this.totalSeguro = this.totalSeguro +this.productosSeleccionados[index].seguro;
      this.totalFlete = this.totalFlete +this.productosSeleccionados[index].flete;
      if(this.reporte.otros+''.length>1){
        this.productosSeleccionados[index].otros = this.productosSeleccionados[index].total*(parseFloat(this.reporte.otros+''))/parseFloat(this.totalBruto+'');
        this.totalOtros = this.totalOtros +this.productosSeleccionados[index].otros;
      }

    });
  }
  addProduct(){
    let producto = this.productos.filter(res=>{return (res.descripcion == this.producto.descripcion);});
    if(producto.length>=1 && this.producto.cantidad.length>=1 && this.producto.bultos.length>=1){
      ...
      this.productosSeleccionados.forEach((element,index) => {
        if(element.idProducto==this.producto.idProducto){
          isAdd=true;
          this.productosSeleccionados[index].cantidad = (parseInt(this.productosSeleccionados[index].cantidad)+parseInt(this.producto.cantidad))+"";
          this.productosSeleccionados[index].bultos = (parseInt(this.productosSeleccionados[index].bultos)+parseInt(this.producto.bultos))+"";
          this.productosSeleccionados[index].total = parseInt(this.productosSeleccionados[index].cantidad)*this.productosSeleccionados[index].peso_bruto;
          this.productosSeleccionados[index].totalNeto = parseInt(this.productosSeleccionados[index].cantidad)*this.productosSeleccionados[index].peso_neto;
        }
        this.totalNeto = this.totalNeto+this.productosSeleccionados[index].totalNeto;
        this.totalBruto = this.totalBruto+this.productosSeleccionados[index].total;
      });
      if(isAdd==false){
        let newProducto = {
   ...
   convertedData(data) {
    ...
    for (let i = 0; i < data.length; i++) {
      let data_simplex = [
        ...
        { content:parseFloat(data[i].total+'').toFixed(2) , styles: { halign: 'center',lineWidth:0 } },
       // { content: data[i].flete, styles: { halign: 'center',lineWidth:0 } },
       
       // { content: data[i].seguro, styles: { halign: 'center',lineWidth:0 } },
       // { content: data[i].fob, styles: { halign: 'center',lineWidth:0 } },
       // { content: data[i].otros, styles: { halign: 'center',lineWidth:0 } },
      ];
      converted_data.push(data_simplex);
    }
    let data_simplex = [
      { content:"Total", styles: { halign: 'center',lineWidth:0 } },
      { content: "", styles: { halign: 'center',lineWidth:0 } },
      { content: "", styles: { halign: 'center',lineWidth:0 } },
      { content: this.reporte.totalBruto, styles: { halign: 'center',lineWidth:0 } },
    //  { content: this.reporte.flete, styles: { halign: 'center',lineWidth:0 } },
     
     // { content: this.totalSeguro, styles: { halign: 'center',lineWidth:0 } },
     // { content: this.totalFob, styles: { halign: 'center',lineWidth:0 } },
     // { content: this.totalOtros, styles: { halign: 'center',lineWidth:0 } },
    ];
    //converted_data.push(data_simplex);
