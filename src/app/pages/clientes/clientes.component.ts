...
import { AddClientesComponent } from '../../shared/components/clientes/add-clientes/add-clientes.component';
import { EditClientesComponent } from '../../shared/components/clientes/edit-clientes/edit-clientes.component';
@Component({
...
export class ClientesComponent implements OnInit {
    ...
    if (val != '' && val != ' ') {
      const f = this.temp.filter(function (d) {
        if (d.nombreEmpresa != '' && d.nombreEmpresa!=null && d.nombreEmpresa!=undefined) {
          return (
              d.nombreEmpresa.toLowerCase().indexOf(val) !== -1 ||
  ...
  add(){
    let dialogRef = this.dialog.open(AddClientesComponent, {
      height: '550px',
      width: '450px',
      data: {},
    });
  ...
  openEdit(data){
    let dialogRef = this.dialog.open(EditClientesComponent, {
      height: '550px',
      width: '450px',
      data: {cliente:data},
    });
